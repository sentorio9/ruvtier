/**
 * MaterialCenterpiece — full-bleed asymmetric "Material is memory" block.
 *
 * Left 55% full-bleed fabric macro (crossfades through up to three slots
 * on a slow loop). Right 45% editorial copy treating the fibre as named,
 * traceable house IP. The origin tag (bottom-left of the image) is an
 * editable text placeholder where the user will later drop the real
 * fibre name and trademark.
 */
  ...
  return (
    <section className="bg-background overflow-hidden">
      <div className="grid grid-cols-1 md:grid-cols-[55fr_45fr]">
        ...
        <div className="relative w-full aspect-[4/5] md:aspect-auto md:min-h-[75vh] overflow-hidden bg-[hsl(30_18%_88%)]">
          {frames.map((src, i) => (
            <img
              key={src + i}
              src={src}
          ...
          {/* Origin / traceability tag — ivory chip, bottom-left */}
          <div className="absolute left-[clamp(16px,2vw,28px)] bottom-[clamp(16px,2vw,28px)]">
            <Editable
              kind="text_block"
              ...
              as="span"
              className="inline-block type-eyebrow tracking-luxury-wide text-foreground bg-[#F7F5F0]/95 px-3 py-1.5 border border-foreground/10 uppercase"
            >
              {originTag}
            </Editable>
          </div>
        ...
        {/* Copy — right */}
        <div className="flex flex-col justify-center bg-background px-[clamp(28px,5vw,72px)] py-[clamp(48px,9vh,96px)]">
          <ScrollFadeIn>
            <Editable
              kind="text_block"
              contentKey="home_material_memory"
            ...
            <Editable
              ...
              as="p"
              className="type-body max-w-[480px] mb-6 md:mb-7 text-foreground/75"
            >
              {body}
            </Editable>
            <Editable
              ...
              as="p"
              className="type-eyebrow tracking-luxury-wide text-foreground/55 mb-8 md:mb-10 uppercase"
            >
              {fibres}
            </Editable>
